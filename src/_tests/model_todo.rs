use crate::{
    model::{self, db::init_db, todo::Todo},
...
#[tokio::test]
async fn model_todo_get_fail() -> Result<(), Box<dyn std::error::Error>> {
    // --FIXTURE
    let db = init_db().await?;
    let utx = utx_from_token("123").await?;
...
async fn model_todo_update_ok() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let utx = utx_from_token("123").await?;
    let data_fx = TodoPatch {
        title: Some("test - model_todo_update ok 1".to_string()),
        status: Some(TodoStatus::Open),
        ..Default::default()
    };
    let todo_fx = TodoMac::create(&db, &utx, data_fx.clone()).await?;
    let update_data_fx = TodoPatch {
        title: Some("test - model_todo_update ok 2".to_string()),
        status: Some(TodoStatus::Open),
        ..Default::default()
    };
